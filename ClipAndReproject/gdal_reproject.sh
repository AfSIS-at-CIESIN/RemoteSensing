...
NextProj () {
	echo To what projection would you like to reproject all the files in $InputDir ? [enter 0-2]
	echo [1] Lambert Azimuthal Equal Area - laea
	echo [2] Geographic Latitude Longitude - latlong
	echo [0] Exit
...
if [ "$count" != 0 ]; then
	...
	echo No .tif files found in $InputDir
	exit
fi
...
while [ $curr -eq $nex ]
do
	printf '\nCURRENT PROJECTION AND NEXT PROJECTION CANNOT BE THE SAME!\n'
	CurrentProj
	curr=$?
	NextProj
	nex=$?
...
do
	...
	#from Cylindrical Equal Area to Lambert
	elif [[ $curr == 4 && $nex == 1 ]]; then	
		echo User has chosen to reproject from Cylindrical Equal Area to Lambert 
		#reproject from Geographic Lat Long to Lambert
		time gdalwarp -overwrite -s_srs '+proj=cea +lon_0=20 +lat_ts=0 +x_0=0 +y_0=0 +no_defs +a=6378137 +rf=298.257223563 +towgs84=0.000,0.000,0.000 +to_meter=1 +datum=wgs84' -t_srs '+proj=laea +lat_0=5 +lon_0=20 +x_0=0 +y_0=0 +ellps=WGS84 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs' -dstnodata 0 $InputDir/$eachFile'.tif' $OutputDir/"$eachFile"_laea'.tif' -multi -wm 5000
		chmod -R 775 $OutputDir/"$eachFile"_laea'.tif'
	
	#combination of projections is not setup, so exit script
	else
		echo The combination of Current Projection: $curr and Next Projection: $nex is not setup.
		break
	fi
done
