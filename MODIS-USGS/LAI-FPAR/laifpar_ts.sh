...
##Written by Sonya Ahamed, modified by Kimberly Peng, Yanni Zhan and John Squires
...
if [ $DatasetName == "Fpar" ] ; then
	...
	#exports to geotiff
	r.out.gdal input='FprAverage@'$mapset output=$OutputDir/"$NasaCode"_"$DatasetName"_"$StartYear"_"$EndYear"_'Average.tif' type=Float32 
	r.out.gdal input='FprVariance@'$mapset output=$OutputDir/"$NasaCode"_"$DatasetName"_"$StartYear"_"$EndYear"_'Variance.tif' type=Float32
	r.out.gdal input='FprStdDev@'$mapset output=$OutputDir/"$NasaCode"_"$DatasetName"_"$StartYear"_"$EndYear"_'StdDev.tif' type=Float32
	...
	#exports to geotiff
	r.out.gdal input='LaiAverage@'$mapset output=$OutputDir/"$NasaCode"_"$DatasetName"_"$StartYear"_"$EndYear"_'Average.tif' type=Float32 
	r.out.gdal input='LaiVariance@'$mapset output=$OutputDir/"$NasaCode"_"$DatasetName"_"$StartYear"_"$EndYear"_'Variance.tif' type=Float32
	r.out.gdal input='LaiStdDev@'$mapset output=$OutputDir/"$NasaCode"_"$DatasetName"_"$StartYear"_"$EndYear"_'StdDev.tif' type=Float32
fi
